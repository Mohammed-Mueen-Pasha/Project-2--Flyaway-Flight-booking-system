	...
	public void insertUser(String userEmail, String fullName, String address, int age, String mobile, String idType,
		...
		try {
			...
			PreparedStatement ps = conn.prepareStatement(
					" insert into regist_details (full_Name,  age, address, mobile, username, id_type, id_no, country ) values (?,?,?,?,?,?,?,?)");
			...
			PreparedStatement ps1 = conn.prepareStatement("SELECT * FROM register_details");
			ResultSet rs = ps1.executeQuery();

			RegisterUser registerUser = new RegisterUser();
			if (rs.next()) {
				System.out.println(rs.getString(2));
				registerUser.setUsername(rs.getString(5));
				registerUser.setFullName(rs.getString(1));
				registerUser.setAddress(rs.getString(3));
				registerUser.setAge(rs.getInt(2));
				registerUser.setMobile(rs.getString(4));

				registerUser.setIdType(rs.getString(6));
				registerUser.setCountry(rs.getString(8));

				registerUser.setIdNumber(rs.getString(7));
